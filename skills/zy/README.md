...
- **[improve-architecture](./improve-architecture/SKILL.md)** —— 架构发现的 SDD 入口：扫描代码库寻找深化机会，以可视化 HTML 报告呈现候选，发布到 issue tracker 的新 feature 文件夹（`.scratch/<YYYY-MM-DD>-architecture-review-<HHmmss>/`）。**只做发现不做设计**（不 propose interfaces）：报告是该 feature 文件夹的首个产物，跑 `/to-prd` 综合 PRD（报告作资源文件），过 SDD 的 **Gate 0**（用户 review PRD）后交接 `/sdd-flow` 从 spec 阶段继续。基于 `/codebase-design` 词汇（格式规范见 [references/html-report.md](./improve-architecture/references/html-report.md)）。与 `/codemap` 互补（后者是代码导航索引，本 skill 找深化机会）；与 `/sdd-flow` 互补（后者从 idea/grill 入口，本 skill 从架构扫描入口）。取代 upstream `/improve-codebase-architecture`（已从 plugin 注销，文件保留）：复用其 Explore + 报告发现，用 SDD 路径（`/to-prd` → `/to-spec` + Gate A）替代其 grilling 设计。
...
- **[sdd-flow](./sdd-flow/SKILL.md)** —— 端到端 SDD 编排器：grill → prd → spec → build → review → maintain。只负责交接、git 检查点（首次写文件前建 `feature/<slug>`/`bugfix/<slug>` 分支；每个 gate 与每个 spec 完成后各 commit）与人工关卡（gate 时指向已落库的 spec 文件让用户 review）；spec 质量由 `/to-spec` 自身保证（拆分走 best-of-N + judge，复杂 spec 设计基于 `/codebase-design` 的 DESIGN-IT-TWICE），不归本 skill。每个阶段都委托给对应 skill。模型调用；自动串起整条流水线（grill → prd → spec → build）—— grill 用模型调用的 `/grilling` + `/domain-modeling` 核心，而非用户调用的 `/grill-with-docs`；`/handoff` 现为模型调用，收尾时直接调用生成下个会话启动 prompt；对用户调用的 `/triage` 只读取 / 建议。
