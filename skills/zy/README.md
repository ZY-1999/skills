...
- **[sdd-flow](./sdd-flow/SKILL.md)** —— 端到端 SDD 编排器：grill → prd → spec → build → review → maintain。只负责交接、spec 上的 best-of-N 自动审查并提交、以及人工关卡；每个阶段都委托给对应 skill。模型调用；自动串起整条流水线（grill → prd → spec → build）—— grill 用模型调用的 `/grilling` + `/domain-modeling` 核心，而非用户调用的 `/grill-with-docs`；对用户调用的 `/triage` 与 `/handoff` 只读取 / 建议。
